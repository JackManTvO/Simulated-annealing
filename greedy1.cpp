...
queue<Cgoods> buffergoods;					//缓冲区货物
...
int main ()
{
	...
	double runtime;
	cout << "贪心算法1" << endl;
	cout << "泊位：由小到大，起吊：分船起吊，由大到小，运送：先入先出" << endl;
	...
	OUT:
		for (int i = 0; i < berthqtt - freeberth; i++)
		{
			if (parkingship[i].lifeline[0] + parkingship[i].intime == totaltime)					//入泊完成
			{
				...
				for (int j = 0; j < parkingship[i].goodsqtt; j++)
					freegoods[inship].push (parkingship[i].sgoods[j]);
				inship++;
			}
		...
		while (freecrane)
		{
			if (!freegoods[outship].empty ())//从freegoods队列移至craninggoods
			{
